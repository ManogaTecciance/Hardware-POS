...
test.describe('WS-4 — Restaurant navigation is derived from the profile', () => {
  ...
  test('WS-408 the till watches the kitchen board but cannot work it', async ({ page }) => {
     ...
     */
    /*
     * Put a ticket on the board first. The original version of this test read
     * whatever tickets a well-used dev database happened to hold and passed on
     * them; on a freshly seeded database the board was empty and the "sees
     * tickets" positive control failed for a reason that had nothing to do
    ...
    await cashierApi.post('/restaurant/takeaway', {
      branchId: RESTAURANT_SEED.branchId,
      idempotencyKey: uniq('ws408'),
      customerName: 'WS-408 board check',
      items: [{ sourceKind: 'PRODUCT', productId: 'prd_resto_10', quantity: 1 }],
    });
