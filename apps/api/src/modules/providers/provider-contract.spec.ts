...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the D67 printing migrations are additive and touch no settlement table', () => {
    const files = [
      '20260829000000_add_auto_printing',
      '20260830000000_add_user_printer_preference',
      '20260831000000_add_branch_default_kitchen_printer',
      '20260901000000_add_print_agent',
      '20260902000000_add_print_job_order',
    ].map((dir) => readFileSync(resolve(MIGRATIONS_DIR, dir, 'migration.sql'), 'utf8'));
    const [autoPrinting, preference, branchDefault, agent, orderBill] = files as [
      string,
      string,
      string,
      string,
    ...
    // POSITIVE — the columns and table auto-printing actually needs.
    expect(autoPrinting).toContain('CREATE TYPE "PrinterRole"');
    expect(autoPrinting).toContain(`ALTER TYPE "PrintJobType" ADD VALUE 'ORDER_BILL'`);
    expect(preference).toContain('CREATE TABLE "UserPrinterPreference"');
    expect(branchDefault).toContain('ADD COLUMN     "defaultKitchenPrinterId" TEXT');
    expect(agent).toContain('CREATE TABLE "PrintAgent"');
    // Leases are what stop the two transports printing the same row twice.
    expect(agent).toContain('ADD COLUMN     "leaseId" TEXT');
    // A bill can precede its Sale (takeaway), so saleId widens to nullable —
    // a widening, which is why no existing row changes meaning.
    expect(orderBill).toContain('ALTER COLUMN "saleId" DROP NOT NULL');

    // NEGATIVE — printing is an EDGE concern: no migration here may touch a
    // settlement, catalogue or order table. The whole point of the outbox is
    // that the money core does not know printers exist.
    const forbidden = ['Sale', 'SaleItem', 'Product', 'RestaurantOrder', 'RestaurantOrderItem'];
    for (const sql of files) {
      for (const table of forbidden) {
        expect({ table, touched: new RegExp(`ALTER TABLE "${table}"`).test(sql) }).toEqual({
          table,
          touched: false,
        });
      }
    ...
    // A preference must degrade, never block: deleting a printer sets the
    // choice back to "use the branch default".
    expect(preference).toContain('ON DELETE SET NULL');
  });
