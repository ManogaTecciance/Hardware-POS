 ...
 *
 * Phase 1.5 has added two so far — `20260806140000_add_role_key` (D40) and
 * `20260806160000_add_role_lifecycle` (Phase 1.5.5). Both are additive and
 * listed explicitly rather than the assertion being relaxed to a count or a
 * prefix match: a new migration must still fail this spec until someone adds
 * its name here, which is the moment they have to say why it exists. The
 * second test additionally proves each Phase 1.5 migration creates no table.
 */
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the two Phase 1.5 recorded', () => {
  ...
  it.each(PHASE_1_5_MIGRATIONS)(
    'the Phase 1.5 migration %s creates no table and drops nothing',
    (name) => {
      // Additive means additive. `Role`, `Permission` and `_RolePermissions` already
      // existed; each Phase 1.5 migration only adds columns and indexes to `Role`.
