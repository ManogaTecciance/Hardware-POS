...
describe('Slice 6C-B adopted the sale, return and product paths, and only those', () => {
  ...
  it('the multi-branch guard is intact on every LOCAL mutator', () => {
    ...
    expect(guarded.sort()).toEqual([
      'adjustStock',
      // D111 (`8.7`) — the sixth, and guarded CONDITIONALLY: only a count that
    ...
    // D111 — the guard inside `applyStockCount` is behind a product-level test,
  ...
  it('ProvidersModule is imported only by the modules that resolve a provider', () => {
    ...
    expect(importers).toEqual([
      ...
      // D111 (`8.7`) — stock takes resolve `InventoryProvider.applyStockCount`
  ...
  it('only the adopted modules import from the providers directory', () => {
    ...
    expect(offenders).toEqual([
      ...
      // D111 (`8.7`) — the count module and the service that drives the port.
      'modules/stock-takes/stock-takes.module.ts',
...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the fifteen post-Slice-4 additions', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // D111 (`8.7`) — stock takes. TWO NEW TABLES AND NOTHING ELSE: no column
      ...
      // D112 (`8.9`) — brand as an entity. One new table plus a NULLABLE
      ...
      // D113 (`6.1`) — weighed goods. ONE NEW ENUM AND TWO ADDITIVE COLUMNS:
      ...
      // D113d (`6.5`) — the unit a line was SOLD in, frozen onto both document
      ...
      // D104 / D104a (5.1) — the tenant option library. Two new tables and two
      ...
      // D104 Part 3 (5.6) — barcode provenance. One enum, one nullable column,
      ...
      // D106 (5.7) — `PrintJob.saleId` widened to nullable so a label sheet can
      ...
      // D107 (7.1a) — the Exchange link table. One new table, nothing else
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // `5.7`), Phase 7 one (`7.1a`), Phase 8 two (`8.7` D111 stock takes, `8.9`
    // D112 brands) and Phase 6 two (`6.1` D113 weighed goods, `6.5` D113d the
    // unit snapshot).
    ...
    // D111 — the two Phase 8 tables, named so that "the count went up" is not
