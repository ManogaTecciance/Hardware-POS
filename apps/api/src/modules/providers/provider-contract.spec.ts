...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 70 listed — the branch’s 60, main’s 8 and fix/table-tab’s 2', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 70 = the exact-set list above: this branch's 60, main's 8 and fix/table-tab's
    // 2. The unnamed `20260831055102_setup1` that branch carried — a re-accepted
    // copy of the D44 FK churn every migration since has stripped — was dropped in
    // the merge (D110), so the disk and the list agree again.
    expect(scanned.length).toBe(70);
