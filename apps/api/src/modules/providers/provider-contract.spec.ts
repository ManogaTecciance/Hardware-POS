...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 61 = the exact-set list above. While the unaccounted-for
    // `20260831055102_setup1` directory exists on disk this stays red at 62,
    // exactly like the exact-set assertion — its owner must add a decision
    // record for it or remove it; adopting it into a count would hide that.
    expect(scanned.length).toBe(61);
