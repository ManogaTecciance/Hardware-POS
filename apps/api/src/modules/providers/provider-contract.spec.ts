...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the eight post-Slice-4 additions', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    for (const dir of dirs) {
      ...
      // Restaurant Phase 2A-2D introduced restaurant configuration through
      // the operational core; Phase 2.5 introduces branch-scoped inventory.
      // Remaining forbidden entries are Phase 6 (KOT / kitchen ticket).
      // `DiningTable` remains as an alternative name that must never appear
      // (the model is `RestaurantTable`); `InventoryBalance` and
      // `InventoryMovement` are alternative names that must never appear
      // for the same reason (models are `BranchInventory` and
      // `StockMovement`).
