...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the thirteen post-Slice-4 additions', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // D101 — the 86 switch: one nullable Product.soldOutAt column. Purely
      // additive; null (every existing row) means available.
      '20260907000000_add_product_sold_out_at',
      // D104 — one joined table, several tabs: a nullable TableSession.tabName
      // so two parties sharing one arrangement are tellable apart on the
      // ticket and the bill. Purely additive; null (every existing row) means
      // the table's own name stands alone, which is what it always meant.
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 62 = the exact-set list above (61 + D104's `add_table_session_tab_name`).
    // While the unaccounted-for `20260831055102_setup1` directory exists on
    // disk this stays red at 63, exactly like the exact-set assertion — its
    // owner must add a decision record for it or remove it; adopting it into a
    // count would hide that. The constant tracks the LIST, never the disk.
    expect(scanned.length).toBe(62);
