 ...
 *
 * Phase 1.5 has added three so far — `20260806140000_add_role_key` (D40),
 * `20260806160000_add_role_lifecycle` (Phase 1.5.5) and
 * `20260806180000_add_branch_access` (Phase 1.5.6). All additive and listed
 * explicitly rather than the assertion being relaxed to a count or a prefix
 * match: a new migration must still fail this spec until someone adds its name
 * here, which is the moment they have to say why it exists. The additive-only
 * loop proves each of the two ALTER TABLE migrations creates no table; the
 * branch-access migration is a CREATE TABLE by design, so it is asserted
 * separately.
 */
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the three Phase 1.5 recorded', () => {
