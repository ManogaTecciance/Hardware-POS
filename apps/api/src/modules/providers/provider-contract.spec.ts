...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 87 listed — the branch’s 60, main’s 8, fix/table-tab’s 2, fix/issues-restaurant’s 1, feature/retail-template’s 13, merge/restaurant-changes’ 1, D171’s 1 and D175’s 1', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // D171 — `Receipt.receiptNumber` becomes unique per TENANT.
      ...
      // D175 — the store-credit ledger. `refundMethod = 'STORE_CREDIT'` was a
      // LABEL on a return and nothing else: no balance was recorded, the
      // customer screen's "Available credit" is a different figure entirely
      // (`creditLimit - outstandingCredit`, what they may buy ON ACCOUNT), and
      // nothing checked a balance when store credit was tendered.
      //
      // PURELY ADDITIVE: one enum and one table. No existing table is
      // altered, no column changes type, no constraint is dropped — which is
      // why the assertion below about creating no provider, inventory or
      // restaurant table still holds. `StoreCreditEntry` is none of those.
      //
      // It also BACKFILLS: every completed STORE_CREDIT return with a saved
      // customer gets its entry, because the shop genuinely owes that money
      // and starting the ledger at zero would erase a real liability on the
      // day the feature that records it shipped.
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 85 = the exact-set list above: this branch's 60, main's 8, fix/table-tab's 2,
    // fix/issues-restaurant's 1, feature/retail-template's 13 (D120–D135) and
    // merge/restaurant-changes' 1 (D147). The unnamed `20260831055102_setup1` two
    // of those branches carried — a re-accepted copy of the D44 FK churn every
    // migration since has stripped — was dropped in D110, so the disk and the
    // list agree.
    // 87 since D175 added `20260918000000_add_store_credit_ledger`.
