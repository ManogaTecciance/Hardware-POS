...
describe('Slice 6C-B adopted the sale, return and product paths, and only those', () => {
  ...
  it('only the adopted modules import from the providers directory', () => {
    ...
    expect(offenders).toEqual([
      ...
      // D101 — the availability/derivation unit spec stubs the
      // CatalogSyncProviderFactory ProductsService already resolves, so it
      // imports the same TYPE the service does.
      'modules/products/product-availability.spec.ts',
      'modules/products/products.module.ts',
...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 87 listed — the branch’s 60, main’s 8, fix/table-tab’s 2, fix/issues-restaurant’s 1, feature/retail-template’s 13, merge/restaurant-changes’ 1, D181’s 1 and D192’s 1', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // main, 2026-08-31 — SALESPERSON joins the UserRole enum (owner-equivalent).
      '20260831090920_add_salesperson_role',
      // D67 — the on-site print agent: the cloud API cannot reach a shop LAN.
      '20260901000000_add_print_agent',
      // main, 2026-09-01 — the business-date index; the feature's D58 migration
      // deliberately no longer creates the same index (merge note, 00-decisions.md).
      '20260901101500_index_sale_completed_at',
      // main, 2026-09-01 — pins every existing business to Asia/Colombo (data only).
      '20260901160000_set_sri_lanka_timezone',
      ...
      // main, 2026-09-04 — Sale.paymentDueDate + its tenant-scoped index.
      '20260904090000_add_sale_payment_due_date',
      // D92 — the synthetic walk-in area's name was showing to waiters.
      '20260905000000_rename_walk_in_area',
      // main, 2026-09-05 — credit becomes an ACCOUNT balance: Payment.saleId
      // widens to optional, Payment.customerId/settledAt, Sale.creditSettledAt.
      '20260905090000_customer_level_credit',
      // D97 — takeaway defaulted off, so creating the config row disabled it.
      '20260906000000_takeaway_enabled_by_default',
      // fix/table-tab, D101 — the 86 switch: one nullable Product.soldOutAt
      ...
      // merge/restaurant-changes, D147 — `KitchenTicket.stationId` becomes
      // NULLABLE. One `ALTER COLUMN ... DROP NOT NULL` and nothing else: no
      // value moves, the foreign key is untouched, and tickets cut before the
      // change keep the station they were genuinely routed to. Widening a
      // column cannot fail on existing data, and the only writer of the column
      // now supplies NULL deliberately.
      '20260916000000_kitchen_ticket_station_optional',
      // D181 — unattended printing returns. Restores, ADDITIVELY, what D68's
      // `20260903000000_kitchen_ticket_completion` dropped: the `PrintAgent`
      // table, `PrinterRole`, the lease columns, the branch's auto-print
      // switches, and ORDER_BILL (plus a new PRINTER_TEST) on the job enum.
      // Nothing dropped, no row touched; `UserPrinterPreference` deliberately
      // stays gone (D152's station decides the device, not the person).
      '20260917000000_d181_restore_printing',
      // D192 — `Receipt.receiptNumber` becomes unique per TENANT.
      // `receiptNumber` is `RCP-<sale.saleNumber>` and `Sale` is
      // `@@unique([tenantId, saleNumber])`, so every tenant's numbering
      // restarts at S-000001 and the GLOBAL unique meant the first tenant to
      // print its sale N permanently blocked every other tenant's sale N with
      // a P2002 surfaced as a 500. Adds `Receipt.tenantId` (nullable,
      // backfilled from the owning sale, then NOT NULL), swaps the global
      // unique for `(tenantId, receiptNumber)` and adds the tenant FK.
      // Creates NO table. The old constraint was strictly stronger than the
      // new one, so no existing row can violate it, and the receipt number
      // FORMAT is unchanged — hardware and restaurant receipts reprint
      // byte-for-byte as before.
      //
      // Shares D181's timestamp. The two were written on different branches in
      // the same week and touch disjoint tables (printing vs Receipt), so their
      // order is immaterial; Prisma orders same-timestamp folders by full name,
      // which puts `d181_` before `scope_` deterministically. Neither had run
      // anywhere when they met, so neither name moved — a folder that has run
      // must never be renamed (D182).
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 87 = the exact-set list above: this branch's 60, main's 8, fix/table-tab's 2,
    // fix/issues-restaurant's 1, feature/retail-template's 13 (D120–D135),
    // merge/restaurant-changes' 1 (D147), D181's 1 and D192's 1. The unnamed
    // `20260831055102_setup1` two of those branches carried — a re-accepted copy
    // of the D44 FK churn every migration since has stripped — was dropped in
    // D110, so the disk and the list agree.
    expect(scanned.length).toBe(87);
   ...
   */
  it('D68 drops every object D67 created, D67 really created them, and D181 puts them back additively', () => {
    ...
    // POSITIVE (D181) — the restore, by name, and ADDITIVE: every D67 object
    // D68 dropped is created again except the one D181 chose not to bring
    // back, and nothing is dropped or deleted on the way.
    const d174 = read('20260917000000_d181_restore_printing');
    expect(d174).toContain('CREATE TABLE "PrintAgent"');
    expect(d174).toContain(`CREATE TYPE "PrinterRole" AS ENUM ('KITCHEN', 'CASHIER')`);
    expect(d174).toContain(`ALTER TYPE "PrintJobType" ADD VALUE IF NOT EXISTS 'ORDER_BILL'`);
    expect(d174).toContain(`ALTER TYPE "PrintJobType" ADD VALUE IF NOT EXISTS 'PRINTER_TEST'`);
    expect(d174).toContain('ADD COLUMN     "autoPrintKot" BOOLEAN NOT NULL DEFAULT true');
    expect(d174).toContain('ADD COLUMN     "defaultKitchenPrinterId" TEXT');
    expect(d174).toContain('ADD COLUMN     "defaultReceiptPrinterId" TEXT');
    expect(d174).toContain('ADD COLUMN     "leaseId" TEXT');
    // The comment header names it (to say why it is absent); the statement must not.
    expect(d174).not.toContain('CREATE TABLE "UserPrinterPreference"');
    expect(d174).not.toMatch(/DROP TABLE|DROP COLUMN|DROP TYPE|DELETE FROM|SET NOT NULL/);

    // The live schema agrees with D181: printing objects present, the
    // per-user preference absent, and — the board half of D181's safety
    // condition — D68's completion columns and statuses still there. Ticket
    // status is the board's; print state lives on the attempt alone.
    ...
    for (const back of [
      ...
      expect({ symbol: back, present: schema.includes(back) }).toEqual({
        symbol: back,
        present: true,
      });
    }
    expect({ symbol: 'model UserPrinterPreference', present: schema.includes('model UserPrinterPreference') }).toEqual({
      symbol: 'model UserPrinterPreference',
      present: false,
    });
