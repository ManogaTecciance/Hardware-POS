...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the five post-Slice-4 additions', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    for (const dir of dirs) {
      ...
      // Restaurant Phase 2A/2B introduced menu configuration tables. The
      // remaining forbidden entries are the OPERATIONAL tables that Phase 2C
      // (dining) and Phase 2D (order lifecycle) will introduce on their own
      // migrations — no migration before those sub-slices lands may pre-empt
      // them.
    ...
    // Phase 2B — Menu is configuration too. `MenuItem` is in the forbidden
    // list because 2C/2D shouldn't recreate it; here it's expected once.
