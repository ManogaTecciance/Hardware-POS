 ...
 *
 * Phase 1.5 adds one — `20260806140000_add_role_key` — under decision D40, with
 * Product Owner authorisation for additive migrations. It is listed explicitly
 * rather than the assertion being relaxed to a count or a prefix match: a new
 * migration must still fail this spec until someone adds its name here, which is
 * the moment they have to say why it exists. The second test additionally proves
 * it creates no table at all.
 */
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the one Phase 1.5 recorded', () => {
  ...
  it('the Phase 1.5 migration creates no table and drops nothing', () => {
    // Additive means additive. `Role`, `Permission` and `_RolePermissions` already
    // existed; this migration only adds a column and an index to one of them.
    const sql = readFileSync(
      resolve(MIGRATIONS_DIR, '20260806140000_add_role_key', 'migration.sql'),
      'utf8',
    );
