...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 69 tracked migration directories. Phase 5 added three (`5.1`, `5.6`,
    // `5.7`) and Phase 7 one (`7.1a`). This deliberately does NOT count the
    // untracked schema-drift folder `20260828081727`, which the tech lead ruled
    // must be left alone — so this assertion keeps failing on exactly that one
    // folder, as it did before Phase 5, rather than being adjusted to
    // accommodate it.
    expect(scanned.length).toBe(69);
