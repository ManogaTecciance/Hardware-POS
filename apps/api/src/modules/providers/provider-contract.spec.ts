...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the six post-Slice-4 additions', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    for (const dir of dirs) {
      ...
      // Restaurant Phase 2A-2C introduced menu and dining configuration.
      // The remaining forbidden entries are the OPERATIONAL tables that
      // Phase 2D (order lifecycle) and Phase 6 (KOT) will introduce on
      // their own migrations. `DiningArea`/`RestaurantTable` are removed
      // because Phase 2C legitimately creates them; `DiningTable` stays
      // because that alternative name must never appear.
