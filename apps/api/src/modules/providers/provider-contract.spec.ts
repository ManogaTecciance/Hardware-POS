...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the fifteen post-Slice-4 additions', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // D120 / D120a (2.1) — the Retail template returns. `ALTER TYPE
      ...
      // D132 (`8.7`) — stock takes. TWO NEW TABLES AND NOTHING ELSE: no column
      ...
      // D133 (`8.9`) — brand as an entity. One new table plus a NULLABLE
      ...
      // D122 (3.8) — per-line tax. Three ADD COLUMNs: `Product.taxable`
      ...
      // D134 (`6.1`) — weighed goods. ONE NEW ENUM AND TWO ADDITIVE COLUMNS:
      ...
      // D134d (`6.5`) — the unit a line was SOLD in, frozen onto both document
      ...
      // D123 (4.1) — promotion allocation: four ADD COLUMNs, shipped inert.
      ...
      // D124 — `PROMOTIONS` becomes its own module key. One ALTER TYPE, no row
      // written with the new value, so D120a's two-migration rule does not apply.
      '20260910000000_add_promotions_module_key',
      // D126 — cart-level FIXED_AMOUNT_DISCOUNT. Six additive columns across
      ...
      // D125 / D125a (5.1) — the tenant option library. Two new tables and two
      ...
      // D125 Part 3 (5.6) — barcode provenance. One enum, one nullable column,
      ...
      // D127 (5.7) — `PrintJob.saleId` widened to nullable so a label sheet can
      ...
      // D128 (7.1a) — the Exchange link table. One new table, nothing else
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 60 + the D120 retail enum migration (2.1) + the D122 per-line tax migration
    ...
    // 73 tracked migration directories. Phase 5 added three (`5.1`, `5.6`,
    // `5.7`), Phase 7 one (`7.1a`), Phase 8 two (`8.7` D132 stock takes, `8.9`
    // D133 brands) and Phase 6 two (`6.1` D134 weighed goods, `6.5` D134d the
    // unit snapshot).
    // This deliberately does NOT count the untracked schema-drift folder
    // `20260828081727`, which the tech lead ruled must be left alone — so this
    // assertion keeps failing on exactly that one folder, as it did before
    // Phase 5, rather than being adjusted to accommodate it.
    expect(scanned.length).toBe(73);
    // D132 — the two Phase 8 tables, named so that "the count went up" is not
    // the only evidence that they are the ones that were added.
    expect(createdTables.has('StockTake')).toBe(true);
    expect(createdTables.has('StockTakeLine')).toBe(true);
    expect(createdTables.has('Brand')).toBe(true);
