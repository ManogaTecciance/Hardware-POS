...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the seven post-Slice-4 additions', () => {
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    for (const dir of dirs) {
      ...
      // the operational core. Remaining forbidden entries are Phase 2.5
      // (branch-scoped inventory) and Phase 6 (KOT / kitchen ticket).
      // `DiningTable` remains as an alternative name that must never appear
      // (the model is `RestaurantTable`).
