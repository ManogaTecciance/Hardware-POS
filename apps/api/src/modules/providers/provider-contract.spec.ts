...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 86 listed — the branch’s 60, main’s 8, fix/table-tab’s 2, fix/issues-restaurant’s 1, feature/retail-template’s 13, merge/restaurant-changes’ 1 and D174’s 1', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // D174 — unattended printing returns. Restores, ADDITIVELY, what D68's
      ...
      '20260917000000_d174_restore_printing',
    ]);
  });
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 86 = the exact-set list above: this branch's 60, main's 8, fix/table-tab's 2,
    // fix/issues-restaurant's 1, feature/retail-template's 13 (D120–D135),
    // merge/restaurant-changes' 1 (D147) and D174's 1. The unnamed
   ...
   */
  it('D68 drops every object D67 created, D67 really created them, and D174 puts them back additively', () => {
    ...
    // POSITIVE (D174) — the restore, by name, and ADDITIVE: every D67 object
    // D68 dropped is created again except the one D174 chose not to bring
    // back, and nothing is dropped or deleted on the way.
    const d174 = read('20260917000000_d174_restore_printing');
    ...
    // The live schema agrees with D174: printing objects present, the
    // per-user preference absent, and — the board half of D174's safety
