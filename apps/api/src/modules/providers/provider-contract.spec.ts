...
describe('no Prisma migration was generated by Slices 5 through 6C-A', () => {
  ...
  it('the migration set is exactly the 20 from Slice 4 plus the thirteen post-Slice-4 additions', () => {
    ...
    expect(migrationDirs()).toEqual([
      ...
      // D106 — the KDS Preparing state: one KitchenTicketStatus enum value
  ...
  it('no migration creates a provider, inventory or restaurant table', () => {
    ...
    // 62 = the exact-set list above (D106 adopted the Preparing enum
    // migration). While the unaccounted-for `20260831055102_setup1`
    // directory exists on disk this stays red at 63, exactly like the
    // exact-set assertion — its owner must add a decision record for it or
    // remove it; adopting it into a count would hide that.
