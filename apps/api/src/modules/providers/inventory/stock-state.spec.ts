/*
 * Lives beside the LOCAL provider rather than under common/: it imports the
 * provider to pin its rollup rule, and the providers-adoption tripwire
 * (provider-contract.spec) treats any importer outside the adopted modules
 * as an adoption — which a spec of the provider itself is not (D119/D136).
 */
import { Prisma } from '@hardware-pos/database';

import { aggregateVariantStock, stockStateFor, type VariantStockCell } from '../../../common/stock-state';
import {
  aggregate,
  aggregateByVariant,
} from './local-inventory.provider';
