...
export class PrintingController {
  ...
  @Get('branch')
  @RequirePermissions(Permission.KITCHEN_STATION_MANAGE)
  branch(@TenantId() tenantId: string, @Query('branchId') branchId: string) {
    return this.printing.branchPrinting(tenantId, branchId);
  }
  ...
  job(@TenantId() tenantId: string, @Param('jobId') jobId: string) {
    return this.printing.jobStatus(tenantId, jobId);
  }
