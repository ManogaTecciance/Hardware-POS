...
import { KitchenPrintAttemptStatus, PrintJobStatus, PrintJobType } from '@hardware-pos/database';
...
export class PrintAgentService {
  ...
  async lease(
    ...
    for (const attempt of attempts) {
      ...
      const payload = await this.dispatcher.renderKotPayload(attempt.ticketId, attempt.printer);
      if (!payload) continue;
    ...
    for (const job of billJobs) {
      ...
      if (!printer) continue;
      // Settled bill (Sale), a bill printed at placement (order), or a test
      // page — the agent gets the same bytes the server would have sent.
      const payload = await this.dispatcher.renderJobPayload(job, printer, 'AGENT');
      if (!payload) continue;
      const leaseId = randomBytes(12).toString('hex');
      ...
      jobs.push({
        ...
        payloadBase64: payload.toString('base64'),
        description:
          job.type === PrintJobType.PRINTER_TEST ? `Test page ${printer.name}` : `Bill ${job.saleId}`,
      });
    }
