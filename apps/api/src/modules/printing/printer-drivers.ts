  ...
  switch (target.kind) {
    ...
    case KitchenPrinterKind.A4_NETWORK:
      ...
      return {
        ok: false,
        error:
          'Office printers print through the on-site agent: pair an agent on the PC where this printer is installed and run it there.',
      };
    default:
 ...
 * and `sendOverTcp` truthfully reports "delivered". The first shop to try
 * this pointed the kitchen at a Canon and read "Test page printed" while
 * nothing came out.
 *
 * DLE EOT 1 is ESC/POS real-time status: a receipt printer answers with one
 * byte, anything else stays silent. Silence is evidence, not proof — some
 * genuine printers ship with status replies disabled — so the caller turns
 * it into a WARNING beside the delivered page, never into a failure.
 */
