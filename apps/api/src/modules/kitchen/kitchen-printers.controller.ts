...
export class KitchenPrintersController {
  ...
  async update(
    ...
    // The links are untouched by an update, but the screen replaces its row
    // with this response — an empty list here would blank them on the spot.
    const links = await this.prisma.kitchenStationPrinter.findMany({
      where: { printerId: updated.id },
   ...
   * error string is the driver's own (socket refused, timed out, bad path),
   * because a generic failure message is useless when configuring hardware.
   * With an on-site agent the page is QUEUED (`queued: true`, `jobId`) and the
   * screen polls `GET /printing/jobs/:jobId` for the agent's verdict.
   */
