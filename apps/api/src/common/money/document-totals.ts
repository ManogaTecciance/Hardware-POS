...
/**
 * What a FIXED amount is measured against (main, 2026-09-07 — per-unit discounts).
 *
 * `LINE` is the meaning every existing caller had: the amount comes off the line
 * once. `UNIT` multiplies it by the quantity first. A percentage has no basis —
 * it is the same figure per unit and per line — and the API refuses a UNIT
 * percentage before it reaches here.
 */
export type DocumentDiscountBasis = 'LINE' | 'UNIT';
...
export interface DocumentLineInput {
  ...
  /** Absent reads as LINE — what every document before this meant. */
  discountBasis?: DocumentDiscountBasis | null;
}
...
/**
 * Money off, which can never exceed the base it applies to.
 *
 * `units` only matters to a FIXED amount on a UNIT basis. Multiply first, round
 * once — `dp2(value × units)`, never `dp2(value) × units` — so a per-unit
 * quotation converts to a sale on the same cent. The clamp is load-bearing: a
 * Rs. 2,000-per-unit discount on a Rs. 1,000 item floors the line at zero rather
 * than going negative, and a negative line would pay money OUT through the
 * proportional reversal in the returns calculation.
 *
 * Exported so the two callers that hold a pre-computed line subtotal (the sale
 * pipeline's `computeDiscount`, the quotation calc) use this arithmetic rather
 * than a float copy of it — the drift D59 replaced.
 */
export function discountAmountOf(
  base: Prisma.Decimal | number,
  type: DocumentDiscountType | null | undefined,
  value: Prisma.Decimal | number | null | undefined,
  opts: { basis?: DocumentDiscountBasis | null; units?: Prisma.Decimal | number } = {},
): Prisma.Decimal {
  if (!type || value == null) return ZERO;
  const b = D(base);
  const v = D(value);
  if (v.lessThanOrEqualTo(0)) return ZERO;
  if (type === 'PERCENTAGE') return Prisma.Decimal.min(b, dp2(b.mul(v).div(100)));
  const units = opts.basis === 'UNIT' ? D(opts.units ?? 1) : D(1);
  return Prisma.Decimal.min(b, dp2(v.mul(units)));
}
...
function discountOf(
  ...
  type: DocumentDiscountType | null | undefined,
  value: Prisma.Decimal | number | null | undefined,
  basis?: DocumentDiscountBasis | null,
  units?: Prisma.Decimal,
): Prisma.Decimal {
  return discountAmountOf(base, type, value, { basis, units });
}
...
export function computeDocumentLine(input: DocumentLineInput): ComputedDocumentLine {
  ...
  const discountAmount = discountOf(lineSubtotal, type, value, input.discountBasis, quantity);
  return {
    unitPrice,
    quantity,
    modifierTotal,
