...
describe('the rest of the line survives the extraction', () => {
  it('carries quantity, discount and approval token', () => {
    ...
    const item: CartItem = {
      ...
      quantity: 3,
      discount: { type: 'FIXED', value: 10, basis: 'UNIT', reason: 'Staff' },
    ...
    expect(toSaleItemPayload(item)).toEqual({
      ...
      discountType: 'FIXED',
      discountValue: 10,
      // main's per-unit basis (2026-09-07). Left out of the payload, the server
      // would read a 10-per-unit discount as 10 off the whole line — a
      // twenty-rupee difference on this cart that no other spec would catch.
      discountBasis: 'UNIT',
  ...
  it('sends no basis when the discount named none — the server applies its default', () => {
    const item: CartItem = {
      ...newCartItem(product(), variant()),
      discount: { type: 'PERCENTAGE', value: 10, reason: 'Staff' },
    };
    expect(toSaleItemPayload(item).discountBasis).toBeUndefined();
    expect(toSaleItemPayload(item).discountType).toBe('PERCENTAGE');
  });
