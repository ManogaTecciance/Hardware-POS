...
/**
 * Maximum sellable quantity for a cart line: the on-hand stock of the size
 * chosen — or of the product itself when it has no sizes — for Inventory items,
 * or null (no cap) for Service / Non-Inventory items, which aren't stock-tracked
 * and can always be sold.
 *
 * Lives here, in the module that computes the line, so the cart cannot answer
 * "what is this line's cap" one way for the quantity stepper and another way
 * for the out-of-stock warning — which is exactly what it used to do.
 */
export function stockCap(
  product: ClientProduct,
  variant: ClientVariant | null = null,
): number | null {
...
export function computeLine(item: CartItem): LineTotals {
  ...
  const cap = stockCap(item.product, item.variant);
  return {
    lineSubtotal,
    discountAmount,
    lineTotal: round2(lineSubtotal - discountAmount),
    // No cap means the line is not stock-tracked, so it can never be short.
