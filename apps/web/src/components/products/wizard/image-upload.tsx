 ...
 * wizard's `image-upload.tsx`, differing only in the target endpoint
 * (`POST /products/image` via `uploadProductImagePreCreate`) so the wizard
 * can capture a photo before the product exists.
 *
 * Accessibility:
...
export function ImageUpload({ session, value, onChange }: Props) {
  ...
  const handleFiles = React.useCallback(
    async (files: FileList | File[] | null) => {
      ...
      setUploading(true);
      try {
        const { imageUrl } = await uploadProductImagePreCreate(session, file);
        onChange(imageUrl);
      ...
      {mode === 'upload' ? (
        ...
        >
          ...
          <p className="mt-3 text-[11px] text-muted-foreground">
            JPG, PNG or WEBP · Max 5 MB · Recommended 1200 x 1200
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="space-y-2 rounded-xl border border-border bg-card p-3">
          ...
          <p className="text-[11px] text-muted-foreground">
            ...
            product page URL is not an image URL.
          </p>
        </div>
      )}
