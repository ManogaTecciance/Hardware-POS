...
| PROD-046 | A retail tenant starts on the clothing list and may replace it (D64/D150) | Retail wizard → Business details on a workspace that has configured nothing; then PUT /v1/products/:id with `attributes: { allergens: "nuts" }` | Fields offered are Material, Fit (Regular/Slim/Relaxed/Oversized), Care instructions, Gender (Men/Women/Unisex/Boys/Girls), Season, all optional — the RETAIL descriptor's own list, served because nothing is stored; `allergens` is refused as an unknown key until the tenant adds it in Settings → Business details (PROD-051) | P | Not Run |
...
| PROD-051 | A retail tenant defines its own business details (D150) | Settings → Business details on a RETAIL workspace: rename Material's label to Fabric, remove Gender (nothing records it), add a Text box "Batch code", add a Dropdown "Storage" with Chilled/Frozen, add a Calendar date "Expiry", Save; reopen Add Product → Business details | Save succeeds; the tab reloads with the six fields and drops the "suggested fields for your business type" note; the wizard's step 2 renders Fabric (text), Fit, Care instructions, Season, Batch code (text), Storage (select with exactly Chilled/Frozen) and Expiry (`type="date"`); `material` is still the stored key, so a product saved before the rename still shows its value | P | Not Run |
| PROD-052 | A business detail in use cannot be taken away (D150) | On the workspace from PROD-051 set a product's Storage to Chilled and save; then in Settings → Business details remove the Storage field and Save; then remove only the Chilled choice and Save; then change Storage's type to Text box and Save; then remove Batch code, which nothing records | Each of the first three is refused with the server's own wording and the count — e.g. `1 product(s) still record "Storage". Clear it on those products before removing the field.` / `1 product(s) are set to "Chilled" for Storage.` — and the editor keeps the operator's edit rather than reloading; removing Batch code succeeds, proving the guard is not "refuse every removal" | N | Not Run |
| PROD-053 | An empty business-details list is a real answer (D150) | Settings → Business details: remove every field (on a workspace whose products record none) and Save; then open Add Product | Save succeeds with "The Add Product form now skips the business details step."; the wizard shows four steps, not five, and no Business details step; the stepper numbering is consistent | P | Not Run |
| PROD-054 | A calendar date is validated as a real day (D150) | With an Expiry date field configured, PUT /v1/products/:id with `attributes: { expiry: "2026-02-31" }`; then `"09/09/2026"`; then `"2026-9-9"`; then `"2026-09-09"` | The first three are refused — 31 February does not exist, and the other two are not the stored notation — and the fourth is accepted; the refusal names the field | N | Not Run |
| PROD-055 | Business details are RETAIL only, on the server as well as the screen (D150/D56) | As a hardware owner GET /v1/products/business-details and PUT it; repeat as a restaurant owner; then open Settings on both | Both verbs are refused 400 `BUSINESS_DETAILS_NOT_CONFIGURABLE` for both business types; neither workspace shows a Business details tab; the hardware wizard still shows no attributes step and the restaurant one is unchanged | N | Not Run |
| PROD-056 | A stored list stops being read once the workspace is no longer retail (D150) | Configure a list on a RETAIL workspace, change its business type to HARDWARE, open Add Product; change it back to RETAIL | Hardware serves its own (empty) schema and the stored list is ignored, not enforced; changing back restores the tenant's list intact — it was never deleted | N | Not Run |
| PROD-057 | The Inventory tab bar shows only the sub-surfaces a workspace has (D151) | Open /products on a RETAIL workspace, then on a HARDWARE one, then on a RESTAURANT one; on each read the tab row | Retail shows all seven — Products, Categories, Promotions, Attributes, Barcodes, Stock, Purchases; hardware shows the same WITHOUT Attributes; restaurant shows them without Attributes AND without Barcodes. Products / Categories / Promotions and the two disabled placeholders are present in every case, so no bar is ever empty | P | Not Run |
| PROD-058 | Hiding a catalogue tab removes no route and no API (D151) | As a restaurant owner navigate directly to /products/attributes and /products/barcodes by URL; then GET /v1/attribute-library | Both pages still render and the API still answers — D125 made the library shared core on permission, not on business type, and D151 hides a tab rather than gating an authority. The tab row on those pages simply has no tab highlighted | N | Not Run |
| PROD-059 | Retail's sale document is the thermal bill, not an A4 sheet (D152) | On a RETAIL workspace open a completed sale; then run a sale through POS → Payment and complete it | The sale page offers **Thermal receipt** and no **Print A4 bill**; the payment screen shows no “Print A4 bill after payment” toggle and completing the sale opens no A4 print window. On a HARDWARE workspace both are still there, unchanged | P | Not Run |
| PROD-060 | Retail keeps the letterhead its quotations need (D152) | On a RETAIL workspace open Settings → Branding, then Layout, then Preview | Branding still offers logo, accent colour, signature and stamp; Layout shows the A4 page size, margins and column toggles **and** a “What prints on the bill” summary; Preview shows the bill preview with roll calibration **and** the A4 document chooser. The chooser offers Quotation, Return and Exchange but **not** Invoice / Bill | P | Not Run |
| PROD-061 | The bill change reaches retail and nobody else (D152) | Repeat PROD-059 and PROD-060 on a HARDWARE workspace and on a RESTAURANT one | Hardware is completely unchanged — A4 bill button, A4 payment toggle, A4 Layout, A4 Preview with Invoice / Bill. Restaurant is completely unchanged — no A4 anywhere, no letterhead controls, bill preview only | N | Not Run |
| PROD-062 | The bill preview shows the workspace's own trade (D153) | Settings → Preview on a RETAIL workspace; then on a RESTAURANT one | Retail's bill sample lists clothing and groceries (Cotton Shirt, Basmati Rice) with a discount and tax but **no service charge and no table number**; restaurant's still lists its menu WITH a service charge and table M1/04. Neither shows the other's | P | Not Run |
| PROD-063 | An unconfigured letterhead is neutral, never another trade (D153) | On a workspace with the Business name field left blank, Settings → Preview → Quotation; then set the Business name and refresh the preview | The blank case shows the tenant's registered name (or “Your Business” if it has none) and **never “Hardware POS”**; once a business name is saved, the preview shows that instead | P | Not Run |
| PROD-064 | The A4 preview shows the workspace's own trade (D154) | Settings → Preview → Quotation on a RETAIL workspace; then on a HARDWARE one; then on a GENERAL one | Retail lists clothing and groceries (Cotton Shirt, Basmati Rice) and **no** hardware goods; hardware lists exactly what it always did (Portland Cement, TMT Steel Bar, all eight, with their SKUs when the SKU column is on); general lists neutral “Standard Item” filler and neither trade | P | Not Run |
| PROD-065 | A vertical that declares no sample goods gets filler, not another trade (D154) | Inspect the preview on any workspace whose domain declares no `sampleItems` | “Standard Item 1”… with realistic prices, a pack line and the discount/tax rows — never another vertical's goods. This is the guard that stops the original defect being rebuilt one level down for every future vertical | N | Not Run |
| PROD-066 | ~~A same-product offer says how close it is, and still lets you pay (D155)~~ **SUPERSEDED by PROD-088 (D160)** | — | This case required Proceed to Payment to stay ENABLED at 5 units. D160 reverses that: an unfinished offer now blocks. Do not run this case — run PROD-088 | — | Superseded |
| PROD-067 | ~~A debt still blocks, and outranks the offer (D155/4.14)~~ **SUPERSEDED by PROD-090 (D160)** | — | This case required that with both offers live only the blocking notice shows — “one instruction at a time”. D160 makes both blocking, so both are named together. Run PROD-090 | — | Superseded |
| PROD-068 | The promotion name placeholder names no vertical (D155) | Products → Promotions → New, on a RETAIL workspace: read the Name field's placeholder | “e.g. Weekend Offer” — **not** “e.g. Lunch Bundle”, which is a restaurant example in a shared editor | P | Not Run |
| PROD-069 | A variant product never reads as single-variant while loading (D156) | Open a product with many variants (e.g. “Test”, 25 options). Watch the At-a-glance card as the page settles; then throttle the network and reload | It shows “Loading…” for Variants and Selling price, then “25 active” and a price range. It must **never** say “Single-variant product” or “Rs. 0.00” — those are the parent's legacy fields, which D44 says are not read once the product has variants | N | Not Run |
| PROD-070 | A failed variant fetch says so instead of inventing a shape (D156) | Open a variant product with the network blocked for `/products/:id/variants` (devtools request blocking) | Variants reads “Could not be loaded” with a hint to reload — not “Single-variant product”. Before D156 the failure was caught and returned an empty array, so it was indistinguishable from a genuine answer and never corrected | N | Not Run |
| PROD-071 | A genuinely single-variant product answers immediately (D156) | Open a product with no variants and watch the At-a-glance card from first paint | “Single-variant product” appears at once with its real price — no “Loading…”. The product payload already says the shape, so a simple product must not wait on a variant list it has no reason to care about | P | Not Run |
| SET-032 | The Preview tab shows one document at a time (D157) | RETAIL workspace → Settings → Preview | A segmented control reads **Printed bill** / **A4 documents**, with Printed bill chosen and the bill below it. The A4 chooser is NOT on screen underneath — before D157 both were stacked and reaching the quotation meant scrolling past a whole receipt | P | Not Run |
| SET-033 | The toggle swaps the document both ways (D157) | On that screen, click **A4 documents**, then **Printed bill** again | A4 documents shows the Document type chooser and the A4 sheet, with no bill; Printed bill brings the bill back and puts the chooser away. Arrow keys move between the two segments as well as clicking | P | Not Run |
| SET-034 | The A4 chooser keeps its document across a flip (D157) | Click **A4 documents**, set Document type to **Return / Refund**, click **Printed bill**, then **A4 documents** again | Still on Return / Refund. The panel is hidden, not thrown away, so comparing a return slip against the bill does not send you back to Quotation each time | P | Not Run |
| SET-035 | The choice survives leaving the tab (D157) | Click **A4 documents**, go to **Branding**, change nothing, come back to **Preview** | Still on A4 documents. The working loop is change the logo → look at the document → change it again; landing back on the bill each time would make that unusable | P | Not Run |
| SET-036 | Hardware and restaurant gain no toggle (D157) | Open Settings → Preview on a HARDWARE workspace, then on a RESTAURANT one | Neither shows a Printed bill / A4 documents control. Hardware still shows its A4 Document type chooser (with Invoice / Bill); the restaurant still shows its bill preview. A one-option toggle is a dead control | N | Not Run |
| PROD-072 | The overview names the product's category (D158) | Open a product that has a category (most seeded products do) → Overview → Product card | A **Category** row reads the category's name, and "Parent › Child" where a subcategory is set. Before D158 the page showed the id nowhere and the name nowhere | P | Not Run |
| PROD-073 | An uncategorised product says so immediately (D158) | Open a product with no category and watch the Product card from first paint | Category reads "—" at once — never "Loading…". The id arrives with the product, so a product with no category must not wait on a catalogue it has no use for | P | Not Run |
| PROD-074 | A slow catalogue does not read as "no category" (D158) | Open a product that HAS a category with `/categories` throttled or blocked (devtools) | Category reads "Loading…", then the name — or "Could not be loaded" if the request fails. It must **never** read "—", which is what an uncategorised product shows and would be a different, wrong fact | N | Not Run |
| PROD-075 | Business details render with the tenant's own labels (D158) | On a RETAIL workspace that has configured business details (Settings → Business details), open a product that has values recorded | A **Business details** card lists each field under the label the tenant typed, with the product's value. A boolean reads Yes / No — not blank, and not "false" | P | Not Run |
| PROD-076 | No empty Business details card (D158) | Open a product with nothing recorded; then open any product on a HARDWARE workspace | No Business details card at all in either case. Hardware's vertical declares no attributes, so the card would be permanently empty there | N | Not Run |
| PROD-077 | The overview shows what a product varies on (D158) | Open a product with variants (e.g. "Test", 25 options) → Overview | A **Variations** card lists each dimension (Size, Colour…) with its options as chips. Before D158 the dimensions were fetched on every page load and shown only inside the variant edit dialog | P | Not Run |
| PROD-078 | A failed variations fetch says so (D158) | Open a variant product with `/products/:id/variations` blocked | The Variations card reads "Could not be loaded. Reload the page to try again." — not an absent card, which would read as "this product varies on nothing" for a product whose payload says it has variants | N | Not Run |
| PROD-079 | Brand shows where a brand is set (D158) | Set a brand on a product via the API (`PATCH /products/:id` with `brandId`), then open it | A **Brand** row names the brand. On a product with no brand there is **no Brand row at all**. NOTE: no screen assigns a brand yet — the wizard has never sent `brandId` — so this needs the API call to observe | P | Not Run |
| PROD-080 | Opening stock actually lands (D159) | Add Product → a simple product (no variations) → Pricing & inventory: set a selling price, an **Opening quantity** of 12 and a cost, pick the **Opening stock branch**, save. Then open the product | Total stock reads 12. The **Inventory** tab shows 12 at that branch, the **Purchases** tab shows an opening receipt, and Average cost is the cost you entered. Before D159 the quantity was validated, shown on Review as "Opening stock", and then silently dropped — the product saved with zero stock | P | Not Run |
| PROD-081 | Opening stock asks where it lands (D159) | On that step, type an Opening quantity and try to continue without choosing a branch | An **Opening stock branch** selector appears as soon as the quantity is entered, and Continue is refused with "Pick where the opening stock lands." With the quantity blank, no selector appears at all | N | Not Run |
| PROD-082 | The SKU Generate button (D159) | On the same step, press **Generate** beside the SKU field | The field fills with the product name as a SKU plus a 3-character suffix (e.g. "Cement 50kg Bag" → `CEMENT-50KG-BAG-A7F`). It stays editable. Pressing it again gives a different suffix | P | Not Run |
| PROD-083 | A blank SKU is now allowed (D159) | Leave the SKU empty and save | The product saves. Before D159 the field carried a red asterisk AND a placeholder offering to generate one if left blank — neither half of which was true, since blank was refused and nothing generated anything | P | Not Run |
| PROD-084 | The SKU length cap survived (D159) | Paste 81 characters into the SKU field | "SKU is limited to 80 characters." Making SKU optional must not have removed the rule the server actually enforces | N | Not Run |
| PROD-085 | Cost and quantity precision (D159) | Enter a cost price of `4.567`, then an opening quantity of `12.3456` | Each is refused with "at most 2 / 3 decimal places". These values now travel to the receipt endpoint, whose line DTO is stricter than the product one; uncapped, the product would be created and then the receipt would 400 | N | Not Run |
| PROD-086 | Variant products still receive opening stock once (D159) | Create a product WITH variations, give two variants an opening quantity, pick the branch, save | Each variant's stock is what you entered — not doubled. The variants batch posts its own opening receipt, and the single-product path deliberately posts nothing for a variant product | N | Not Run |
| PROD-087 | The wizard counts its own steps (D159) | Walk through every step of Add Product on a RETAIL workspace, reading the small label above each heading | It reads "Step 1 of 5" … "Step 5 of 5", matching the dots above. Before D159 four of the five said "of 4" — D150 added Business details as a fifth step and only that step's label was computed. On a workspace whose domain declares no business details, the same walk reads "of 4" correctly | P | Not Run |
| PROD-088 | A same-product offer blocks until it is complete (D160) | RETAIL till: with a "buy 5 get 1" on ONE product, add 4 of it, then 5, then 6 | At 4 nothing. At **5** a 🎁 card names the offer, says "Add 1 <product> to complete this offer" and **Proceed to Payment is DISABLED** with "Payment is unavailable until the offer is complete." At 6 the card goes, the discount applies and payment is enabled. This REVERSES PROD-066 (D155), which required payment to stay enabled at 5 | N | Not Run |
| PROD-089 | The notice is identical for both shapes (D160) | Compare the card at 5 units of a same-product offer against the card for a "buy 2 shirts get 1 tie" with 2 shirts and no tie | Same 🎁, same "Add N ... to complete this offer" sentence, same "Payment is unavailable" line, same colour. The point of D160 is that a cashier cannot tell the two kinds apart, because they now require the same thing | P | Not Run |
| PROD-090 | Both offers are named when both are short (D160) | Have a same-product offer AND a cross-product offer both incomplete in one basket | The card lists **both**, each with its own 🎁 and name. D155 hid the same-product one while a debt was live; hiding one now would leave the cashier completing an offer and finding payment still blocked with no reason shown | P | Not Run |
| PROD-091 | A small basket is not held hostage (D160) | With a "buy 5 get 1" live, add just 1 or 2 units and try to pay | No card, payment enabled. The customer has qualified for nothing, so there is nothing to complete. If this blocks, D160 has been over-applied and every small sale in the shop is refused | N | Not Run |
| PROD-092 | The count is right, not always "one" (D160) | With a "buy 2 get 2" offer, put exactly 2 in the basket | "Add **2** <product>s to complete this offer" — two, plural. A till that always said "1" passed every other case here until this one was added | P | Not Run |
| PROD-093 | ACCEPTED COST — exactly five cannot be sold (D160) | With "buy 5 get 1", a customer who wants exactly 5 and refuses the free 6th | The sale **cannot be completed** at 5. This is the decision, not a defect: the cashier must add the 6th (which is free) or remove one to sell 4. Recorded so it is not re-filed as a bug | N | Not Run |
...
| DOC-055 | A reprint keeps the tender (D165) | Reprint the DOC-050 receipt from Sales history, then reprint it a SECOND time | Both show **Cash** and **Balance** with the original figures. D162 recorded this as impossible without a migration; it was not — the first print already stored the number in `Receipt.content`, and the reprint was erasing it. A receipt printed BEFORE D162 still shows nothing, which is correct | P | Not Run |
...
| DOC-057 | The receipt shows four rows, nothing repeated (D164) | Print a cash receipt with change (DOC-050) and count the money rows | Exactly **Total / Cash / Balance / Status**. No `Paid` row, no `Balance Rs. 0.00`, and the total appears **once** — before D164 it appeared three times (Total, Paid and a trailing Cash payment row) | P | Not Run |
| DOC-058 | A split tender keeps its full breakdown (D164) | Pay a sale with Card + Cash together | `Paid` / `Balance` and BOTH method rows are printed. The four-row layout is only for a single cash payment, because on a split the change is not tender minus total | N | Not Run |
| DOC-059 | A retail till offers only the receipt (D166) | RETAIL workspace → POS → complete a payment and read the dialog | **Print receipt** is the single, primary print button. No **Print A4 Bill**, no **Preview A4 Bill**. Before D166 the A4 pair was primary and the receipt was a small text link between them | N | Not Run |
| DOC-060 | A hardware till keeps both A4 actions (D166) | Same on a HARDWARE workspace | **Preview A4 Bill** and **Print A4 Bill** are both there, with the thermal receipt as the secondary link. D152 took the A4 from retail only | P | Not Run |
| DOC-061 | The receipt prints itself after payment (D166) | RETAIL, with *Print receipt after payment* ON, complete a sale | The thermal receipt prints on its own, as the A4 does on hardware. Before D166 the auto-print was gated on the A4 existing, so a retail sale printed **nothing** | P | Not Run |
