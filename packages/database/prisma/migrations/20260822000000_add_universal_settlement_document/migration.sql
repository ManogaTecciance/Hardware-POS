...
-- Sale(tenantId, completedAt): NOT created here. `20260901101500_index_sale_completed_at`
-- (main, 2026-09-01) creates this exact index and had already been deployed when the
-- restaurant branch was merged. Two CREATE INDEX statements for one name fail whichever
-- runs second, so this — the migration that had never left the repository — gives it up.
-- The schema declares the index once; both chains end with it present.
